...
int main(void) {
    ...
    int a5[] = {1, 2, 4, 8, 16, 32, 64, 8, 9, 10};
    int a6[] = {1, 2, 4, 8, 16, 32, 64, 128, 9, 10};
    int a7[] = {1, 2, 4, 8, 16, 32, 64, 128, 256, 10};
    int a8[] = {1, 2, 4, 8, 16, 32, 64, 128, 256, 512};


    arrays[0] = a0; arrays[1] = a1; arrays[2] = a2; arrays[3] = a3;
    arrays[4] = a4; arrays[5] = a5; arrays[6] = a6; arrays[7] = a7;
    arrays[8] = a8;

    int arrayOp[number_arrays];
    for (i = 0; i < number_arrays; i++) {
        printf("Result: %s", detectionEquation(arrays[i], n) == 1? "True" : "False");
        printf("\tOperations: %d\n",number_operations);
        arrayOp[i] = number_operations;
    }

    double valorMedio = 0;
    for(i = 0; i < number_arrays; i++){
        printf("%d\n",arrayOp[i]);
        valorMedio += arrayOp[i] * (1.0/9.0);
    }
