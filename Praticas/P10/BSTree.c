...
struct _BSTreeNode {
  void* item; // informacao
  struct _BSTreeNode* left; // ponteiro para info menor
  struct _BSTreeNode* right; // ponteiro para info maior
  int height; // quantos nos tenho para baixo
};

struct _BSTreeHeader {
  unsigned int numNodes; // numero de nós
  struct _BSTreeNode* root; // Nó da root
  compFunc compare; // funcao para comparar
  printFunc print; // funcao para imprimir
};
...
static int _treeGetHeightREC(const struct _BSTreeNode* root) {
  ...
  int left = _treeGetHeightREC(root->left);
  int right = _treeGetHeightREC(root->right);
...
static void* _treeGetMax(const struct _BSTreeNode* root) {
  ...
  else return _treeGetMax(root->right);
}
...
void* BSTreeSearch(const BSTree* header, const void* item) {
  ...
  while (current != NULL) {
    // COMPLETE THE LOOP.
    comp = header->compare(item, current->item);
    if (comp == 0) return current->item;
...
static void _BSTreeAddItems(const struct _BSTreeNode* p, Queue* q) {
  ...
  // Assim fica ordenado
  // Mudando esta ordem tudo muda!
  // ISTO SAI SEMPRE NO TESTE TEORICO !!!!!!!!
  if (p->left != NULL) _BSTreeAddItems(p->left,q);
  QueueEnqueue(q,p->item);
  if (p->right != NULL) _BSTreeAddItems(p->right,q);
